...
function user_login_callback($type, $user)
{
	...
	$_SESSION['user_wx_openid'] = $user['user_wx_openid'];
	$_SESSION['invite_code'] = $user['invite_code'];
...
function add_moneylog($user_id, $type, $money, $text = null, $time = '')
{
	...
	if (in_array($type, array('recharge', 'add', 'back', 'tg', 'recovery'))) {
...
function useraddr_list($address_id)
{
	...
	foreach ($info_list as $k => $v) {
		...
		$info['user_phone'] = userbank_num($info['user_phone']);
	}
