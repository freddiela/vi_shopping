...
switch($act) {
	...
	case 'agent_login':
		if (isset($_p_pesubmit)) {
			...
			if ($info = $db->pe_select('user', " and (user_name = '{$user_name}' or user_email = '{$user_name}' or user_phone = '{$user_name}') and user_pw = '{$user_pw}' and agent = 1")) {
			    if($info['user_status'] == 1){
                    pe_jsonshow(array('result'=>false, 'show'=>'对不起，此账号已冻结，请联系客服'));
                }
				user_login_callback('login', $info);
				pe_jsonshow(array('result'=>true, 'show'=>'登录成功'));
			}
			else {
				pe_jsonshow(array('result'=>false, 'show'=>'帐号或密码错误或不是代理'));
			}
		}
        //在线客服
        $service = $db->pe_select('setting', array('setting_key'=>'online_service'));
		$seo = pe_seo($menutitle='代理登录');
 		include(pe_tpl('agent_login.html'));
	break;
	//####################// 用户登录 //####################//
	case 'login':
		if (isset($_p_pesubmit)) {
	...
	case 'register':
		...
		if (isset($_p_pesubmit)) {
			...
			if ($_p_user_pw1 && $_p_user_pw != $_p_user_pw1) pe_jsonshow(array('result'=>false, 'show'=>'两次密码不一致'));			if($_p_invite_code){
                $tguser = $db->pe_select('user', array('invite_code'=>$_p_invite_code), 'user_id');
                if ($tguser) pe_setcookie('tguser_id', intval($tguser['user_id']));
            }
			...
			$sql_set['invite_code'] = create_invite_code();
