...
switch($act) {
    ...
    //####################// 拼团详情 //####################//
    case 'pintuan':
        $order_id = pe_dbhold($_g_pintuan_id);
        $info = $db->pe_select('order', array('order_id'=>$order_id));
        if (!$info['order_id']) pe_error('参数错误...');
        $qrcode = pe_qrcode("{$pe['host_root']}index.php?mod=order&act=pintuan&id={$order_id}");
        echo "<div class=\"pro_ewm\"><img src=\"{$qrcode}\" style='width: 210px;' /></div>";
        break;
	//####################// 订单列表 //####################//
	default:
