...
//记录用户操作时间
$db->pe_update('user', array('user_id'=> $_s_user_id), array('user_operation_time'=> time(),'user_from'=> $_SERVER['HTTP_HOST']));
...
//更新活动中商品
$info_list = $db->index('product_id')->sql_selectall("select * from (select * from `".dbpre."huodongdata` where `huodong_stime` <= '{$nowtime}' and `huodong_etime` > '{$nowtime}' and `huodong_type` != 'taocan' order by product_money asc) a group by product_id");
