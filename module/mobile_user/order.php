...
switch($act) {
	...
	case 'view':
        ...
        $recovery = $db->pe_select('order_recovery', array('order_id'=>$order_id));
	...
	case 'close':
		...
		if (isset($_p_pesubmit)) {
			...
			if($info['order_cancel'] == 1){
				$sql_set['order_state'] = 'apply_close';
		...
		foreach ($info_list as $k=>$v) {
		    ...
		    if($product_list){
		        foreach ($product_list as &$p){
                    $product  = $db->pe_select('product', array('product_id'=>$p['product_id']),'product_r_day');
                    if($product['product_r_day']){
                        $p['product_r_date'] = $v['order_ptime'] + $product['product_r_day'] * 24 * 3600;
                        if(time() >= $p['product_r_date']){
                            $p['product_r_date'] = 1;
                        }
                    }else{
                        $p['product_r_date'] = 0;
			...
			$info_list[$k]['product_list'] = $product_list;
