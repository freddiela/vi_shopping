...
switch ($act) {
	...
	//case 'add_pintuan':
        ...
        if($_g_huodong_id){
            $cart = taocan_list($_g_huodong_id,$cart_id);
            $cart['info']['huodong_id'] = $_g_huodong_id;
        }else{
		...
		if (isset($_p_pesubmit)) {
            ...
            if($info['order_type'] == 'pintuan'){
                $log = $db->pe_select('pintuanlog', array('pintuan_id'=>$info['pintuan_id'], 'user_id' => $_s_user_id));
                if($log){
                    pe_jsonshow(array('result'=>false, 'show'=>'您不能重复参加同一个拼团'));
                }
            }
    ...
    //####################// 拼团详情 //####################//
    case 'pintuan':
        $pintuan_id = pe_dbhold($_g_id);
        $info = $db->pe_select('pintuan', array('pintuan_id'=>$pintuan_id));
        if (!$info['pintuan_id']) pe_error('参数错误...');
        $log = $db->pe_selectall('pintuanlog', array('pintuan_id'=>$pintuan_id, 'order by'=>'pintuanlog_id asc'));
        $surplus = $info['pintuan_num'] - count($log);
        if($log){
            foreach ($log as $k=>$v){
                if($k==0){
                    $log[$k]['pintuan_leader'] = true;
                }else {
                    $log[$k]['pintuan_leader'] = false;
                }
            }
        }
        $seo = pe_seo($menutitle='拼团订单详情');
        include(pe_tpl('tuan_view.html'));
        break;
	//####################// 快递查询跳转 //####################//
	case 'kuaidi':
