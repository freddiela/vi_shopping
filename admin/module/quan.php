...
switch ($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			...
			$_p_info['product_id'] = $_p_info['product_id'] ? implode(',',$_p_product_id) : '';
			...
			if ($quan_id = $db->pe_insert('quan', pe_dbhold($_p_info))) {
				if($_p_info['quan_type']=='offline'){
					...
					for($i=0;$i<intval($_p_info['quan_num']);$i++){
						//$randomstr = substr(pe_guid(),-5).substr(mt_rand(0,9).pe_guid(),-5);
						$randomstr = substr(pe_guid(),-10)+$i+rand(0,9);
						$sql .= "(".$_p_info['quan_atime'].",".$quan_id.",'".$randomstr."','".$_p_info['quan_name']."',".$_p_info['quan_money'].",".$_p_info['quan_limit'].",'".$_p_info['quan_sdate']."','".$_p_info['quan_edate']."','".$_p_info['product_id']."'),";
						$randomstr = "";
					}
	...
	case 'edit':
		...
		if (isset($_p_pesubmit)) {
			pe_token_match();
			$_p_info['product_id'] = implode(',',$_p_product_id);
