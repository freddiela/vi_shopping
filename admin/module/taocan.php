...
switch ($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			pe_token_match();
            if ($_FILES['huodong_pic']['size']) {
                pe_lead('public/class/upload.class.php');
                $upload = new upload($_FILES['huodong_pic']);
                $_p_info['huodong_pic'] = $upload->filehost;
            }
			...
			$_p_info['huodong_type'] = $_p_huodong_type ? $_p_huodong_type : 'taocan';
			...
			if($huodong_id && is_array($product_ids)){
                foreach($product_ids as $k=>$v){
                    $db->pe_update('product', array('product_id'=>$v), "huodong_id = '{$huodong_id}', huodong_type = '{$_p_info['huodong_type']}', huodong_tag = '{$_p_info['huodong_tag']}'");
			...
			pe_success('添加成功!', 'webadmin.php?mod=taocan');
		}
        $info_list = array();
		$seo = pe_seo($menutitle='优惠套餐', '', '', 'admin');
        $activity_time_list = $db->pe_selectall('activity_time', array('is_deleted'=>0,'is_active' => 1,'order by' => 'id asc'));
		include(pe_tpl('taocan_add.html','admin'));
	break;
	//####################// 套餐修改 //####################//
	case 'edit':
		...
		if (isset($_p_pesubmit)) {
			...
			pe_success('修改成功!', 'webadmin.php?mod=taocan');
		}
	...
	case 'product_add':
		...
		if($product_id){
			...
			$html = '<tr class="js_product product_id" id="'.$info['product_id'].'">
	...
	//####################// 列表 //####################//
	default :
        $sqlwhere = " and `huodong_type`='taocan'";
		...
		$seo = pe_seo($menutitle='优惠套餐', '', '', 'admin');
		include(pe_tpl('taocan_list.html','admin'));
	break;
}
?>