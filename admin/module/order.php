...
switch ($act) {
	...
	case 'automatic_delivery':
		pe_token_match();
		$data = [
			'setting_value' => $_p_automatic_delivery * -1
		];
		$db->pe_update('setting', array('setting_key' => 'automatic_delivery'), $data);
		break;
		//####################// 订单删除 //####################//
	case 'apply_close':
		pe_token_match();
		...
		if ($info['order_state'] != 'apply_close') pe_error('非申请取消订单...');
		if($_g_status == 'wpay'){
			$sql_set['order_state'] = 'wpay';
			$result = $db->pe_update('order', array('order_id' => $order_id), pe_dbhold($sql_set));
		}else{
			order_callback_close($info, '');
		}
		pe_success('操作成功!');
		break;
		//####################// 订单详情 //####################//
	...
	case 'send_edit':
		...
		if (isset($_p_pesubmit)) {
			...
			if ($info['order_virtual']) {
				...
				$sql_set['order_wl_id'] = $_p_order_wl_id;
				$sql_set['order_wl_name'] = $_p_order_wl_name;
				$result = $db->pe_update('order', array('order_id' => $order_id), pe_dbhold($sql_set));
			}
			if ($result) {
		...
		include(pe_tpl('order_send_edit.html', 'admin'));
		break;
		//####################// 订单发货 //####################//
	case 'send':
		$order_id = pe_dbhold($_g_id);
	...
	case 'batchsend':
		pe_token_match();
		$ids = pe_dbhold($_p_ids);
		if (empty($ids)) {
			pe_error('未选中订单');
		}
		$order_ids = implode(',', $ids);
		$list = $db->pe_selectall('order', " and order_id in({$order_ids}) ");
		if ($list) {
			foreach ($list as $v) {
				//if ($v['order_state'] != 'wsend') continue;
				if ($v['order_virtual']) {
					$result = order_callback_virtual($v);
				} else {
					$result = order_callback_send($v, '', '');
				}
			}
			pe_success('批量发货成功!');
		}
	...
	case 'batchsuccess':
		...
		if ($list) {
			foreach ($list as $v) {
				//if ($v['order_state'] != 'wget') continue;
				order_callback_success($v);
			}
			pe_success('批量确认收货成功!');
		}
		break;
	...
	case 'print_express':
		...
		//####################// 订单列表 //####################//
	default:
		$orderby_arr['money|desc'] = '订单金额（多到少）';
		$orderby_arr['money|asc'] = '订单金额（少到多）';
		$orderby_arr['atime|desc'] = '订单时间（新到旧）';
		$orderby_arr['atime|asc'] = '订单时间（旧到新）';
		...
		if ($_g_orderby) {
			...
			$sql_where .= " `order_{$orderby[0]}` {$orderby[1]},";
		}
		$sql_where .= " `order_id` desc";
		if ($act == 'excel_out') {
			...
			$automatic_delivery = $db->pe_select('setting', array('setting_key' => 'automatic_delivery'));
			$automatic_delivery = $automatic_delivery['setting_value'];
