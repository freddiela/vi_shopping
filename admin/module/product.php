...
switch ($act) {
	...
	case 'edit_zhekou':
		if (isset($_p_pesubmit)) {
			...
			$rmoneys = $_p_product_rmoney;
			$product_num = $_p_product_num;

			if (is_array($product_ids)) {
				foreach ($product_ids as $k => $v) {
					$info = $db->pe_select('product', array('product_id' => $v));
					$sql_update['product_rmoney'] = $rmoneys[$v];
					$sql_update['product_num'] = $product_num[$v];
					$db->pe_update('product', array('product_id' => $v), pe_dbhold($sql_update));
				}
			}
			pe_success('修改成功!', 'webadmin.php?mod=product&act=edit_zhekou');
		}
		$info_list = $db->pe_selectall('product', '', '*');
		$seo = pe_seo($menutitle = '修改折扣', '', '', 'admin');
		include(pe_tpl('product_edit_zhekou.html', 'admin'));
		break;
		//####################// 商品添加 //####################//
	case 'add':
		if (isset($_p_pesubmit)) {
	...
	default:
		...
		$orderby_arr['atime|desc'] = '发布时间（新到旧）';
		$orderby_arr['atime|asc'] = '发布时间（旧到新）';
		$orderby_arr['money|desc'] = '商品价格（高到低）';
		$orderby_arr['money|asc'] = '商品价格（低到高）';
		$orderby_arr['rmoney|desc'] = '回收价格（高到低）';
		$orderby_arr['rmoney|asc'] = '回收价格（低到高）';
		$orderby_arr['r_day|desc'] = '回收时间（新到旧）';
		$orderby_arr['r_day|asc'] = '回收时间（旧到新）';
