	...
	case 'user':
		if (isset($_p_pesubmit)) {
			...
			$sql = "update `".dbpre."setting` set `setting_value` = case `setting_key`
				...
				when 'cashout_is_need_order' then '".intval($_p_info['cashout_is_need_order'])."'
				when 'tg_state' then '".intval($_p_info['tg_state'])."'
				when 'tg_fc1' then '".round($_p_info['tg_fc1'], 2)."'
				when 'tg_fc2' then '".round($_p_info['tg_fc2'], 2)."'
				when 'tg_fc3' then '".round($_p_info['tg_fc3'], 2)."' else `setting_value` end";
			if ($db->sql_update($sql)) {
				cache_write('setting');
				pe_success('设置成功!');
			}
	...
	default:
		if (isset($_p_pesubmit)) {
			...
			$sql = "update `".dbpre."setting` set `setting_value` = case `setting_key` {$sqlset}
				...
				when 'online_service' then '".pe_dbhold($_p_info['online_service'])."'
