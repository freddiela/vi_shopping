...
function pe_tpl($tplname, $tplpath = '')
{
	...
	$tplpathcache = "{$pe['path_root']}cache/template/{$module}/";
	$tplpathcache_name = "{$tplpathcache}{$tplnamearr[0]}.php";
	$tplpath_name = $tplpath ? $pe['path_admintpl'] . $tplname : $pe['path_tpl'] . $tplname;
	...
	if (!is_file($tplpathcache_name) or @filemtime($tplpath_name) > @filemtime($tplpathcache_name)) {
		if (!is_file($tplpath_name)) {
			pe_bug("模板文件 ./template/{$module}/{$tplname} 丢失", __LINE__);			
		}
...
//	$img = str_ireplace($pe['host_root'], $pe['path_root'], $img);
	...
	switch ($thumbtype) {
		case 'avatar':
			$img_new = is_file($img) ? $img : "{$pe['path_root']}public/image/noavatar.jpg";		
		break;
		case 'upload':
			$img_new = is_file($img) ? $img : "{$pe['path_root']}public/image/noupload.jpg";		
		break;
		default :
			$img_new = is_file($img) ? $img : "{$pe['path_root']}public/image/nopic.png";	
		break;
	}
	if ($w or $h) {
		$img_thumb = "{$pe['path_root']}cache/thumb/".date('Y-m')."/thumb_{$w}x{$h}_".pe_filename($img_new);
		if (!is_file($img_thumb)) {
			if ($sington == false) {
				include_once("{$pe['path_root']}public/class/thumb.class.php");
			}
...
function pe_comment($val, $width=16) {
	...
	for ($i=1; $i<=5; $i++) {
		if ($i <= intval($val)) {
			$html .= "<img src='{$pe['host_root']}public/plugin/raty/images/star-on.png' title='{$i}' style='width:{$width}px;margin-right:1px' />";
		}
		elseif (ceil($val) == $i) {
			$html .= "<img src='{$pe['host_root']}public/plugin/raty/images/star-half.png' title='{$i}' style='width:{$width}px;margin-right:1px' />";			
		}
		else {
			$html .= "<img src='{$pe['host_root']}public/plugin/raty/images/star-off.png' title='{$i}' style='width:{$width}px;margin-right:1px' />";	
		}
	}
	return $html;
}
...
{
	if ($type == 'admin') {
		$seo['title'] = $title ? "{$title}": '还没有设置标题呢';
	}
	else {
		$setting = cache::get('setting');
		$seo['title'] = $title ? "{$title} - {$setting['web_title']}" : "{$setting['web_title']}";
...
print<<<html
	...
	#msgshow_l{background:url({$pe['host_root']}public/image/{$bg_name}_l.gif) no-repeat; width:38px; height:50px; float:left;}
	#msgshow_r{background:url({$pe['host_root']}public/image/{$bg_name}_r.gif) no-repeat; width:7px; height:50px; float:left;}
	#msgshow_m{background:url({$pe['host_root']}public/image/{$bg_name}_m.gif) repeat-x; height:34px; float:left; padding:16px 10px 0 10px; font-size:14px; font-weight:bold; color:{$font_color}; display:inline-block; min-width:95px; _width:95px;}
	</style>
...
function pe_token_set($str='') {
	if ($_SESSION['pe_token']) {
		...
		return md5("{$_SERVER['REMOTE_ADDR']}myllop+qiyego=xiaoyao".microtime(true).rand(1000,100000).$str);	
	}
...
function pe_qrcode($url) {
	global $pe;
	$user_qrcode = "{$pe['path_root']}cache/thumb/".date('Y-m')."/".md5($url).".png";
	if (!is_file($user_qrcode)) {
		if (!is_dir("{$pe['path_root']}cache/thumb/".date('Y-m'))) {
			mkdir("{$pe['path_root']}cache/thumb/".date('Y-m'), 0777, true);		
		}
		pe_lead('public/class/phpqrcode.class.php');
		QRcode::png($url, $user_qrcode);
	}
	return "{$pe['host_root']}cache/thumb/".date('Y-m')."/".md5($url).".png";
}
...
function create_invite_code(){
    ...
    $user_info = $db->pe_select('user', array('invite_code' => $d));
    if ($user_info) {
        create_invite_code();
    }
    return $d;
